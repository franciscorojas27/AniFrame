...
import { VideoView, VideoSource, useVideoPlayer, VideoPlayer } from 'expo-video'
...
export default function VideoScreen() {
  ...
  const videoSource: VideoSource = {
    ...
    contentType: videoData?.result.foundUrl.includes('m3u8') ? 'hls' : 'auto',
    useCaching: true,
  }
  ...
  const Element = ({ item, index }: { item: any; index: number }) => {
    return (
      ...
      >
        <Image style={styles.imageItem} source={{ uri: item.capThumbnail }} />
    ...
    <SafeAreaView>
      {videoData?.result ? (
        <View style={styles.container}>
          ...
          />
          <View accessible={false} focusable={false} style={styles.videoInfo}>
            <Image
